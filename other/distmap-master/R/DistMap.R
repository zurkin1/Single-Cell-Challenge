...
setMethod(f = "computeVISH",
          ...
          function(object, gene, threshold) {
            #gene.expr is the level of a specific gene for all given cells (e.g. 1290 cells).
            gene.expr <- as.numeric(object@data[gene, ])
            #Multiplying the gene value for a given cell by the MCC score that this cell recieved (which was maximal).
