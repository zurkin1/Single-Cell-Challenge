...
df1=pd.read_csv("c:\Work\src\Dream\Single cell\other\\PSO_in_TensorFLow\\x_y_data.csv")   #Importing data
col1 = df1['X']
col2 = df1['Y']
x1 = col1.as_matrix()
y1_ = col2.as_matrix()
...
def swarm(P=10):  #Define partilces
    ...
    V_W = tf.Variable(tf.zeros([P]))   #Particle velocities
...
#Update particles
V_W_update = V_W[i].assign( V_W[i]+(Wpbest[i] - Wpc[i])*c1*np.random.random() +(Wglobal[0] - Wpc[i])*c2*np.random.random() ) #Operation for updating V_W[i].
V_b_update = V_b[i].assign( V_b[i]+(bpbest[i] - bpc[i])*c1*np.random.random() +(bglobal[0] - bpc[i])*c2*np.random.random() )
Wp_update = Wpc[i].assign(V_W[i]+Wpc[i]) #Updating Wpc[i].
...
with tf.Session() as sess:
    ...
    for j in range(N):      #Loop over N iterations
        for k in range(P):  #Loop over P particles
            sess.run([V_W_update,V_b_update],feed_dict={i:k}) #Update velocity.
            sess.run([Wp_update,bp_update],feed_dict={i:k}) #Update position.
            sess.run([update_fitness_pc],feed_dict={i:k,x:x1,y_:y1_}) #Update particle error.
            sess.run([pbest_update],feed_dict={i:k}) #Update particle best position.
            sess.run([global_update],feed_dict={i:k}) #Update global best position.
        if j%50== 0:
            print("Iteration:",j)    
